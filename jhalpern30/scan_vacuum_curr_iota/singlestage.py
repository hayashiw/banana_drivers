"""Singlestage (BoozerLS, weighted L-BFGS-B) for scan_vacuum_curr_iota.

Vacuum scan: input biotsavart_opt.json contains only TF (20) + banana (10)
coils. No proxy, no VF, no plasma current → BoozerSurface I = 0.

Truncated ramp: mpol/ntor ∈ [6, 8, 10] with order ∈ [3, 4, 4] and
qp ∈ [192, 256, 256]. No mpol=12 stage (vacuum field convergence is
expected to be marginal even at mpol=10).

Output:
    BANANA_OUT_DIR/stage_NN/bsurf_init.json
    BANANA_OUT_DIR/stage_NN/bsurf_opt.json     (or bsurf_failed.json)
    BANANA_OUT_DIR/stage_NN/state.json
    BANANA_OUT_DIR/stage_NN/summary.json
    BANANA_OUT_DIR/bsurf_opt.json              (copied from final stage on success)
"""
...
VOL_TARGET = 0.10
CONSTRAINT_WEIGHT = 1.0e+3   # BoozerLS surface-label penalty
...
# Weighted L-BFGS-B weights (lift from jhalpern30/singlestage.py)
LENGTH_WEIGHT = 5e-2
RES_WEIGHT = 1e3
IOTAS_WEIGHT = 1e4
...
# Truncated ramp [mpol, ntor, order, qp]
RAMP = [
...
# Per-stage convergence tolerances by mpol (from jhalpern30/singlestage.py)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument('--iota-target', type=float, required=True,
                   help='BoozerLS iota target (signed; positive for the iota>0 basin).')
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f'scan_vacuum_curr_iota singlestage  iota_target={iota_target:+.4f}')
    print(f'BS_FILE = {bs_file}')
    print(f'OUT_DIR = {out_dir}')
    start_time = time.time()

    # Load BiotSavart and target surface
    bs = load(bs_file)
    ...
    surf_coils = banana_curves[0].surf  # CWS winding surface

    # G0 sign fix for TF<0 (see CLAUDE.md "TF sign convention")
    ...
    for stage_idx, stage in enumerate(RAMP):
        ...
        # Persist outputs
        bs_set_points = bsurf.surface.gamma().reshape((-1, 3))
        bs.set_points(bs_set_points)
    ...
    # Promote final stage's bsurf_opt.json to top level if full ramp converged
