...
from simsopt.geo import CurveXYZFourier, SurfaceRZFourier, SurfaceXYZTensorFourier, BoozerSurface, curves_to_vtk, CurveLength, LpCurveCurvature
...
def initialize_boozer_surface(surf_prev, mpol, ntor, bs, vol_target, constraint_weight, iota, G0, I):
    """
...
bsurf_init = load("../bsurf_opt.json")
surf = bsurf_init.surface
bs = bsurf_init.biotsavart.set_points(surf.gamma().reshape((-1, 3)))
...
vol_target = 0.10
CONSTRAINT_WEIGHT = 1e3 # 1.0
MAXITER = 300
num_tf_coils = 20

PLASMA_CURRENT_KA = -1.0
print(f"Plasma current: {PLASMA_CURRENT_KA} kA")

MU0 = 4 * np.pi * 1e-7
I0 = (PLASMA_CURRENT_KA*1e3) * MU0
...
banana_coils = coils[num_tf_coils:num_tf_coils+10]
...
print(f"\n Generating proxy coil")
R_proxy, _ = find_rax(bs, surf)
Z_proxy = 0.0
...
boozer_surface = initialize_boozer_surface(surf, mpol, ntor, bs, vol_target, CONSTRAINT_WEIGHT, iota_target, G0, I0)
...
CURVATURE_THRESHOLD = 50 # can go up to 100
...
JCurveCurve = CurveCurveDistance(curves[20:30], CC_DIST)
JCurveSurface = CurveSurfaceDistance(curves[20:30], boozer_surface.surface, CS_DIST)
