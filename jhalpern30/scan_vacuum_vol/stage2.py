"""Stage 2 (weighted) for scan_vacuum_vol.

Vacuum scan over volume targets. The orchestrator passes --vmec-s as the
scan axis; this driver builds the target surface at that s and the volume
of THAT surface is the BoozerSurface volume target downstream. Banana
current is pinned at -16 kA (hardware cap, sign for the iota=+0.15 basin
under TF<0). TF stays at -80 kA. Optimization is shape-only.

Output:
    BANANA_OUT_DIR/biotsavart_opt.json
    BANANA_OUT_DIR/stage2_summary.json   (includes 'volume_target' field)
    BANANA_OUT_DIR/stage2.log            (orchestrator-tee'd)
"""
...
TF_CURRENT_KA = -80.0
BANANA_CURRENT_KA = -16.0   # pinned (HW cap, sign for iota=+0.15 basin under TF<0)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument('--vmec-s', type=float, required=True,
                   help='VMEC s value at which to extract the target surface '
                        '(scan axis; volume_target = surf.volume() at this s).')
    return p.parse_args(argv)


def _resolve_out_dir(vmec_s: float) -> str:
    env = os.environ.get('BANANA_OUT_DIR')
    if env:
        return env.rstrip('/') + '/'
    fallback = os.path.join(
        os.getcwd(), f'scan_vacuum_vol_s{vmec_s:.4f}',
    )
...
# ──────────────────────────────────────────────────────────────────────────
def build_target_surface(vmec_s: float) -> SurfaceRZFourier:
...
def build_banana_coils(winding_surface) -> tuple[CurveCWSFourierCPP, list[Coil]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = _resolve_out_dir(args.vmec_s)
    os.makedirs(out_dir, exist_ok=True)

    print(f'scan_vacuum_vol stage 2  vmec_s={args.vmec_s:.4f}  '
          f'TF={TF_CURRENT_KA} kA  Ib={BANANA_CURRENT_KA} kA  vacuum')
    print(f'OUT_DIR = {out_dir}')

    target_surf = build_target_surface(args.vmec_s)
    volume_target = float(target_surf.volume())
    print(f'volume_target = {volume_target:.6f} m^3 (from surf at s={args.vmec_s})')
    ...
    summary = {
        ...
        # Scan-axis derived quantity — orchestrator reads this back into the CSV
        'volume_target': volume_target,
        'vmec_s': float(args.vmec_s),
    }
    with open(os.path.join(out_dir, 'stage2_summary.json'), 'w') as fh:
        json.dump(summary, fh, indent=2)
    print(f'wrote {bs_path}')
    print(f'wrote stage2_summary.json (volume_target={volume_target:.6f})')
    return 0
