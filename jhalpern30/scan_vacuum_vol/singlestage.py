"""Singlestage (BoozerLS, weighted L-BFGS-B) for scan_vacuum_vol.

Vacuum scan: input biotsavart_opt.json contains TF (20) + banana (10)
coils only. iota_target is fixed at 0.15 (orchestrator passes it). The
volume target comes from stage 2's surface at the per-point vmec_s
value.
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument('--vmec-s', type=float, required=True,
                   help='VMEC s for the seed surface (matches stage 2).')
    p.add_argument('--volume-target', type=float, required=True,
                   help='Target volume for BoozerSurface (from stage 2 output).')
    p.add_argument('--iota-target', type=float, required=True,
                   help='BoozerLS iota target (typically 0.15 for vacuum_vol).')
...
def main(argv: list[str] | None = None) -> int:
    ...
    vol_target = float(args.volume_target)
    out_dir = _resolve_out_dir()
    os.makedirs(out_dir, exist_ok=True)

    print(f'scan_vacuum_vol singlestage  vmec_s={vmec_s:.4f}  '
          f'vol_target={vol_target:.6f}  iota_target={iota_target:+.4f}')
