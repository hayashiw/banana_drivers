...
def interpolate_target_to_surface(
    surface,
    target_in,
):
    ...
    qpts_phi_in = np.linspace(0, 1/surface.nfp, nphi_in, endpoint=False)
    qpts_theta_in = np.linspace(0, 1, ntheta_in, endpoint=False)

    dbl_qpts_phi_in = np.concatenate([qpts_phi_in, qpts_phi_in + 1/surface.nfp])
